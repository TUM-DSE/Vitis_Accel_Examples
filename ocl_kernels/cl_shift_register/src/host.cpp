...
int main(int argc, char** argv) {
    ...
    OCL_CHECK(err, cl::Buffer buffer_signal_A(context, CL_MEM_READ_ONLY, size_in_bytes,
                                              nullptr, &err));
    OCL_CHECK(err, cl::Buffer buffer_coeff_A(context, CL_MEM_READ_ONLY, coeff_size_in_bytes,
                                             nullptr, &err));
    OCL_CHECK(err, cl::Buffer buffer_output_A(context, CL_MEM_WRITE_ONLY, size_in_bytes,
                                              nullptr, &err));
    OCL_CHECK(err, cl::Buffer buffer_signal_B(context, CL_MEM_READ_ONLY, size_in_bytes,
                                              nullptr, &err));
    OCL_CHECK(err, cl::Buffer buffer_coeff_B(context, CL_MEM_READ_ONLY, coeff_size_in_bytes,
                                             nullptr, &err));
    OCL_CHECK(err, cl::Buffer buffer_output_B(context, CL_MEM_WRITE_ONLY, size_in_bytes,
                                              nullptr, &err));
    ...
    for (int i = 0; i < (n_warmup + n_reps) / 2; i++) {
        ...
        OCL_CHECK(err, err = q.enqueueWriteBuffer(buffer_signal_A, CL_FALSE, 0, size_in_bytes,
                                                  signal.data(), nullptr, &event_data_to_fpga));
        OCL_CHECK(err, err = q.enqueueWriteBuffer(buffer_coeff_A, CL_FALSE, 0, coeff_size_in_bytes,
                                                  coeff.data(), nullptr, &event_data_to_fpga_2));
        ...
        OCL_CHECK(err, err = q.enqueueReadBuffer(buffer_output_A, CL_FALSE, 0, size_in_bytes,
                                                 out.data(), nullptr, &event_data_to_host));
        OCL_CHECK(err, err = q.finish());
    ...
    for (int i = 0; i < (n_warmup + n_reps) / 2; i++) {
        auto t_xpu_0 = std::chrono::high_resolution_clock::now();
        OCL_CHECK(err, err = q.enqueueWriteBuffer(buffer_signal_B, CL_FALSE, 0, size_in_bytes,
                                                  signal.data(), nullptr, &event_data_to_fpga));
        OCL_CHECK(err, err = q.enqueueWriteBuffer(buffer_coeff_B, CL_FALSE, 0, coeff_size_in_bytes,
                                                  coeff.data(), nullptr, &event_data_to_fpga_2));
        OCL_CHECK(err, err = q.finish());
        auto t_xpu_1 = std::chrono::high_resolution_clock::now();
        time_xpu += std::chrono::duration_cast<std::chrono::nanoseconds>(t_xpu_1 - t_xpu_0).count();
        ...
        auto t_xpu_4 = std::chrono::high_resolution_clock::now();
        OCL_CHECK(err, err = q.enqueueReadBuffer(buffer_output_B, CL_FALSE, 0, size_in_bytes,
                                                 out.data(), nullptr, &event_data_to_host));
        OCL_CHECK(err, err = q.finish());
        auto t_xpu_5 = std::chrono::high_resolution_clock::now();
        time_xpu += std::chrono::duration_cast<std::chrono::nanoseconds>(t_xpu_5 - t_xpu_4).count();
